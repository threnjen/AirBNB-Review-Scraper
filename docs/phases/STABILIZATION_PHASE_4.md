# Phase 4: Integration Tests and QA

**Status:** Planned  
**Dependencies:** Phase 3 (Data Extraction & Aggregation)  
...
class TestDataExtractor:
    """Test data extraction from property summaries."""

    def test_extractor_initializes(self):
        """Test DataExtractor initializes correctly."""
        from review_aggregator.data_extractor import DataExtractor
        extractor = DataExtractor(zipcode="97067")
        assert extractor.zipcode == "97067"
        logger.info("✓ DataExtractor initializes correctly")
...
4. At least 5 property summaries in `property_generated_summaries/`
5. Cache directory exists: `cache/summaries/`
...
**Objective:** Verify DataExtractor extracts and aggregates numeric data from summaries
...
1. Set `config.json`:
   ...
   ```
2. Run: `python main.py`
3. Check logs for "Extracting data from X property summaries"
4. Verify `area_data_97067.json` created

**Expected Result:**
- Pipeline completes without errors
- Data file created with structure showing categories, mentions, totals
- Per-property breakdown preserved
...
### Test Case 5: Error Handling - Missing Files
...
2. Set `config.json` to `extract_data: true` with unused zipcode
...
### Test Case 6: Integration Test Suite
...
- Test PropertyRagAggregator, AreaRagAggregator, DataExtractor
- Verify Phase 1-3 implementations work correctly
```
