"""Tests for compile_comp_sets() in steps/03_comp_sets.py — merging per-listing
...
# steps.03_comp_sets uses a dotted name that's not a valid Python identifier,
# so we import it dynamically.
_step_module = import_module("steps.03_comp_sets")
compile_comp_sets = _step_module.compile_comp_sets

ZIPCODE = "97067"


class TestCompileCompSets:
    """Tests for compile_comp_sets()."""
    ...
    def test_merges_single_comp_set(self, comp_sets_dir):
        ...
        compile_comp_sets(ZIPCODE, output_dir=str(comp_sets_dir))

        master_path = comp_sets_dir / f"comp_set_{ZIPCODE}.json"
    ...
    def test_merges_multiple_comp_sets(self, comp_sets_dir):
        ...
        result = json.loads((comp_sets_dir / f"comp_set_{ZIPCODE}.json").read_text())
        assert len(result) == 2
        assert "listing_1" in result
        assert "listing_2" in result

    def test_first_write_wins_on_duplicates(self, comp_sets_dir):
    ...
    def test_no_compset_files_produces_empty_master(self, comp_sets_dir):
    ...
    def test_master_file_does_not_include_itself(self, comp_sets_dir):
        ...
        (comp_sets_dir / f"comp_set_{ZIPCODE}.json").write_text(
    ...
    def test_preserves_all_ten_fields(self, comp_sets_dir):
