"""Tests for compile_comp_sets() in main.py — merging per-comp-set JSON files
...
class TestCompileCompSets:
    ...
    def comp_sets_dir(self, tmp_path):
        """Create a temp directory simulating outputs/01_comp_sets/."""
        d = tmp_path / "outputs" / "01_comp_sets"
        d.mkdir(parents=True)
        return d

    def test_merges_single_comp_set(self, aggregator, comp_sets_dir):
        """A single compset file is written verbatim to the master file."""
        data = {
            "listing_1": {"ADR": 100.0, "Occupancy": 50, "Max_Guests": 4},
            "listing_2": {"ADR": 200.0, "Occupancy": 70, "Max_Guests": 6},
        }
        (comp_sets_dir / "compset_111.json").write_text(json.dumps(data))
    ...
    def test_merges_multiple_comp_sets(self, aggregator, comp_sets_dir):
        """Multiple compset files are merged into one master file."""
        data_a = {
        ...
        (comp_sets_dir / "compset_111.json").write_text(json.dumps(data_a))
        (comp_sets_dir / "compset_222.json").write_text(json.dumps(data_b))
    ...
    def test_first_write_wins_on_duplicates(self, aggregator, comp_sets_dir):
        ...
        # compset_111 sorts before compset_222
        (comp_sets_dir / "compset_111.json").write_text(json.dumps(data_a))
        (comp_sets_dir / "compset_222.json").write_text(json.dumps(data_b))
    ...
    def test_no_compset_files_produces_empty_master(self, aggregator, comp_sets_dir):
        """If no compset files exist, the master file is an empty dict."""
        aggregator.compile_comp_sets(output_dir=str(comp_sets_dir))
    ...
    def test_master_file_does_not_include_itself(self, aggregator, comp_sets_dir):
        """The master comp_set_{zipcode}.json should not be read as a compset input."""
