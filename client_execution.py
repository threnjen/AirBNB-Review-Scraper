import json
from scraper import airbnb_scraper
from main import RagDescription
...
if scrape_reviews == True:
    airbnb_scraper(zipcode = zipcode, iso_code = iso_code, num_listings = number_of_listings_to_process)
    print(f"Scraping reviews for zipcode {zipcode} in country {iso_code} completed.")

if aggregate_reviews == True:
    rag_description = RagDescription()
    rag_description.rag_description_generation_chain()
    print(f"Aggregating reviews for zipcode {zipcode} in country {iso_code} completed.")
